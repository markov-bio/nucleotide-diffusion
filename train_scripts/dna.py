...
# %%
dataset = load_dataset("simecek/Human_DNA_v0")['train']

split_size=128*6 # it's just the max_lenghts times the lenght of the tokens 
def chunk_examples(examples):
    chunks = []
    for sentence in examples["Seq"]:
        chunks += [sentence[i:i + split_size] for i in range(0, len(sentence), split_size)]
    return {"chunks": chunks}


# %%
split_dataset = dataset.map(chunk_examples, batched=True, remove_columns=["Seq"], num_proc=24) # more threads
...
trainer=composer.Trainer(
    ...
    device='gpu',
    callbacks=callbacks,
    loggers=WandBLogger(project='dna_diffusion', group='markovbio'),
